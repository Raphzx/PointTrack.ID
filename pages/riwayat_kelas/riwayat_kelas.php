...
$filter_tahun = isset($_GET['tahun_pelajaran']) ? mysqli_real_escape_string($connect, $_GET['tahun_pelajaran']) : '';
$filter_kelas = isset($_GET['kelas']) ? mysqli_real_escape_string($connect, $_GET['kelas']) : '';

$conditions = [];
if ($keyword) {
    $conditions[] = "(s.nama_siswa LIKE '%$keyword%' OR s.nis LIKE '%$keyword%')";
}
if ($filter_tahun) {
    $conditions[] = "rk.tahun_pelajaran_id = '$filter_tahun'";
}
if ($filter_kelas) {
    $conditions[] = "rk.kelas_id = '$filter_kelas'";
}
$where_clause = $conditions ? "WHERE " . implode(" AND ", $conditions) : "";

$query = "
SELECT rk.id, s.nis, s.nama_siswa, k.nama_kelas, k.tingkat, tp.nama_tahun
FROM riwayat_kelas rk
INNER JOIN siswa s ON rk.siswa_id = s.id
INNER JOIN kelas k ON rk.kelas_id = k.id
INNER JOIN tahun_pelajaran tp ON rk.tahun_pelajaran_id = tp.id
$where_clause
ORDER BY tp.nama_tahun DESC, k.nama_kelas ASC, s.nama_siswa ASC
LIMIT $start, $limit
";
...
<div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Riwayat Kelas</h2>
        <p class="text-sm text-slate-400 mt-1">
            Total <span class="font-bold text-indigo-600"><?= $total_data ?></span> data
        </p>
    </div>
...
<form method="GET"
    ...
    <input type="hidden" name="page" value="riwayat_kelas">
...
<tr class="bg-slate-50/50 text-slate-400 text-[11px] uppercase tracking-widest font-bold">
    ...
    <th class="px-8 py-5 text-center">Kelas & Tingkat</th>
...
<div>
<p class="font-bold text-slate-800"><?= htmlspecialchars($item['nama_siswa']) ?></p>
<p class="text-[11px] text-slate-400">NIS: <?= $item['nis'] ?></p>
</div>
</div>
</td>

<td class="px-8 py-5 text-slate-600 text-center"><?= $item['nama_tahun'] ?></td>

<td class="px-8 py-5 text-center">
<div class="font-bold text-slate-700 mb-1"><?= $item['nama_kelas'] ?></div>
<span class="inline-block px-2.5 py-0.5 rounded-lg text-[10px] font-bold <?= $badge ?>">
