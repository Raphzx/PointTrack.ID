...
$query = "SELECT * FROM aturan_poin ORDER BY min_poin ASC";
$result = mysqli_query($connect, $query);
$total_data = mysqli_num_rows($result);

function getWarnaClass($warna) {
    switch(strtolower($warna)) {
        case 'emerald': return 'bg-emerald-50 text-emerald-600 ring-emerald-200';
        case 'yellow': return 'bg-yellow-50 text-yellow-600 ring-yellow-200';
        case 'orange': return 'bg-orange-50 text-orange-600 ring-orange-200';
        case 'rose': return 'bg-rose-50 text-rose-600 ring-rose-200';
        case 'slate': return 'bg-slate-50 text-slate-700 ring-slate-200';
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-500">
                            Total <span class="font-bold text-indigo-600"><?= $total_data ?></span> aturan terdaftar
                        </p>
                    </div>

                    <a href="aturan_poin_tambah"
                ...
                <table class="min-w-full text-sm text-left">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-400 text-[11px] uppercase tracking-widest font-bold">
                            <th class="px-8 py-5">Rentang Poin</th>
                            <th class="px-8 py-5">Tindakan</th>
                            <th class="px-8 py-5 text-center">Warna Badge</th>
                                ...
                                <td class="px-8 py-5">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-slate-800 text-base">
                                            <?= $item['min_poin'] ?> - <?= $item['max_poin'] ?>
                                        </span>
                                    </div>
                                </td>
                                <td class="px-8 py-5">
                                    <span class="font-bold text-slate-700">
                                        <?= htmlspecialchars($item['tindakan']) ?>
                                    </span>
                                </td>
                                <td class="px-8 py-5 text-center">
                                    <span class="inline-flex items-center px-4 py-1.5 rounded-xl text-xs font-bold <?= getWarnaClass($item['warna']) ?> ring-1 ring-inset ring-current/10">
                                        <?= ucfirst(htmlspecialchars($item['warna'])) ?>
                                    </span>
                                </td>
                                <td class="px-8 py-5 text-right">
                                    <div class="flex justify-end items-center gap-2">
                                        <a href="aturan_poin_edit?id=<?= $item['id'] ?>" 
                                        ...
                                        <a href="aturan_poin_hapus?id=<?= $item['id'] ?>" 
                            ...
                            <tr>
                                <td colspan="4" class="px-8 py-20 text-center">
                                    ...
                                    <p class="text-slate-400 font-medium italic">
                                        Belum ada aturan poin yang diinputkan.
                                    </p>
                                </td>
                            </tr>
                        <?php endif; ?>
